...
params:
    ...
    corpus_cases_dir: !expr file.path("..", "IPBES_TCA_Corpus", "data", "tca_corpus", "corpus_cases")
    #
    ...
    fig_publications_over_time_yearly: !expr file.path("figures", "ch2_technology", "publications_over_time_yearly")
    #
    temporal_from: 1900
...
The input is in parquet format, containing 124518 entries, including publications’ abstracts, OA ID, and publication year. 

The output of the model is 4 scores: compound, negative, neutral, and positive.
It is also in parquet format containing 124518 entries, including, OA ID, publication year, compound score, negative score, neutral score, and positive score.
...
##| # | |
if (length(list.files(path = dirname(params$fig_publications_over_time), pattern = basename(params$fig_publications_over_time_yearly))) < 2) {
...
if (length(list.files(path = file.path("figures", "ch2_technology"), pattern = "sentiments_over_time_neg_pos_ridge")) < 2) {
    figure <- readRDS(params$fn_sentiment_results) |>
        select(
            work_id = id,
            year = date,
    ...
    ggplot2::ggsave(
        file.path("figures", "ch2_technology", "sentiments_over_time_neg_pos_ridge.pdf"),
        width = 12,
        height = 6,
        figure
    )
    ggplot2::ggsave(
        file.path("figures", "ch2_technology", "sentiments_over_time_neg_pos_ridge.png"),
        width = 12,
        height = 6,
        figure
    )
...
![Publications over time of different corpora](figures/ch2_technology/publications_over_time_yearly.png)

A pdf of the graph can be downloaded [here](figures/ch2_technology/publications_over_time_yearly.pdf){target=_blank}.

## `vision AND technology AND marine` in Technology Corpus Corpus

For the Marine subset of the Technologyn Corpus, we do have 
WILL BE ADDED.
...
![](`r file.path("figures", "ch2_technology", "sentiments_marine_over_time_neg_pos_ridge.png")`)
...
![](`r file.path("figures", "ch2_technology", "sentiments_over_time_neg_pos_ridge.png")`)

To download high resolution, [click here](`r file.path("figures", "ch2_technology", "sentiments_over_time_neg_pos_ridge.pdf")`){target="_blank"}
