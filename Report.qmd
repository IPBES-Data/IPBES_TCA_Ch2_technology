...
vision_st <- readLines(file.path("input", "ch2_technology", "vision.txt")) |>
    paste(collapse = " ")

technology_st <- readLines(file.path("input", "ch2_technology", "technology.txt")) |>
    paste(collapse = " ")


pages_dir <- file.path(".", "data", "ch2_technology", "pages")
corpus_dir <- file.path(".", "data", "ch2_technology", "corpus")
corpus_tca_dir <- file.path(".", "data", "ch2_technology", "corpus_tca")
...
IPBES.R::corpus_download(
    pages_dir = file.path(".", "data", "ch2_technology", "pages"),
    title_and_abstract_search = compact(paste0("(", vision_st, ") AND (", technology_st, ")")),
    continue = TRUE,
...
fn <- file.path("data", "ch2_technology", "ids_subs_tca.rds")
if (!file.exists(fn)) {
    ids_technology <- IPBES.R::corpus_read(file.path("data", "ch2_technology", "corpus")) |>
        dplyr::select(id) |>
        collect() |>
        unlist()
    ...
    saveRDS(ids_subs_tca, fn)

    IPBES.R::corpus_filter_ids(
        arrow_dir = corpus_dir,
        arrow_filter_dir = corpus_tca_dir,
...
fn <- file.path("data", "ch2_technology", "sent_analysis_technology.parquet")
if (!file.exists(fn)) {
...
fn <- file.path("data", "ch2_technology", "sent_analysis_technology_tca.parquet")
if (!file.exists(fn)) {
...
- works in the technology corpus AND in the TCA corpus which can be downloded [here](data/ch2_technologyrandom_50_technology_in_tca.xlsx){target=_blank}. 
